<template>
    <div class="fillcontain">
        <search-item @showDialog="showAddFundDialog" @searchList="getMoneyList"
            @onBatchDelMoney="onBatchDelMoney" @put="put"></search-item>
            <!-- <el-button type="primary" @click="put">输出excel</el-button> -->
        <div class="table_container">
            <el-table v-loading="loading" :data="tableData" style="width: 100%" align='center' @select="selectTable"
                ...
                <el-table-column prop="incomePayType" label="收支类型" align='center' width="130" :formatter="formatterType"
                    :filters="fields.incomePayType.filter.list" :filter-method="filterType">
                </el-table-column>
            ...
            <pagination :pageTotal="pageTotal" @handleCurrentChange="handleCurrentChange"
                ...
                <addFundDialog v-if="addFundDialog.show" :isShow="addFundDialog.show" :dialogRow="addFundDialog.dialogRow"
    ...
    methods: {
        ...
        put() {
            ...
            getMoneyIncomePay(para).then(res => {
                let data = res.data.moneyList
                this.put_excel(data, this.list, '梁敏强的表格.xlsx')
            })


        },
//excel 表输出
        put_excel(data, header, fileName) {
            let list = header;
            data.map((item) => {
                const arr = [];
                arr.push(...[item.username, item.address, item.createTime, item.incomePayType, item.income, item.pay]);
                list.push(arr);
            });
            console.log(list);
